...
class CKFinder_Connector_CommandHandler_DeleteFiles extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
        ...
        if ( !CKFinder_Connector_Utils_FileSystem::unlink($filePath) ){
          ...
          @unlink($thumbPath);
            ...
            foreach ($config['thumbs'] as $k=>$v){
                self::delete_thumb($v['max_width'],$v['max_height'],$sServerDir,$name);
            }

            self::delete_original($sServerDir,$name);
  ...
  private function delete_thumb($max_width,$max_height,$sServerDir,$name){
      $thumbPath = CKFinder_Connector_Utils_FileSystem::combinePaths($sServerDir.'../resize/'.$max_width.'x'.$max_height.'/', $name);
      @unlink($thumbPath);
  }
  private function delete_original($sServerDir,$name){
      $thumbPath = CKFinder_Connector_Utils_FileSystem::combinePaths($sServerDir.'../original/', $name);
      @unlink($thumbPath);
  }
