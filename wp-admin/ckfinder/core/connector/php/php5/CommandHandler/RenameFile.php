...
class CKFinder_Connector_CommandHandler_RenameFile extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    protected function buildXml()
    {
        ...
        $newFileName = CKFinder_Connector_Utils_FileSystem::convertToAscii($_GET["newFileName"]);
        ...
        if (!$bMoved) {
            ...
            global $config;
            foreach ($config['thumbs'] as $k=>$v){
                self::rename_file($sServerDir, $v['max_width'], $v['max_height'], $fileName, $newFileName);
            }
            self::rename_original_file($sServerDir, $fileName, $newFileName);
        }
    }

    protected function rename_file($sServerDir,$max_width,$max_height, $fileName, $newFileName){
        $thumbPath = CKFinder_Connector_Utils_FileSystem::combinePaths($sServerDir.'../resize/'.$max_width.'x'.$max_height.'/', $fileName);
        $newThumbPath = CKFinder_Connector_Utils_FileSystem::combinePaths($sServerDir.'../resize/'.$max_width.'x'.$max_height.'/', $newFileName);
        @rename($thumbPath, $newThumbPath);
    }
    protected function rename_original_file($sServerDir, $fileName, $newFileName){
        $thumbPath = CKFinder_Connector_Utils_FileSystem::combinePaths($sServerDir.'../original/', $fileName);
        $newThumbPath = CKFinder_Connector_Utils_FileSystem::combinePaths($sServerDir.'../original/', $newFileName);
        @rename($thumbPath, $newThumbPath);
    }
}
